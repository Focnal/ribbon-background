...
const RibbonBackground: React.FC<AnimatedBackgroundProps> = React.memo(
  ({ zIndex = -1, alpha = 0.5, size = 100, excludedSelectors = [] }) => {
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      // 比较 excludedSelectors 的内容
      prevProps.excludedSelectors?.join() ===
        nextProps.excludedSelectors?.join()
    );
  }
);
